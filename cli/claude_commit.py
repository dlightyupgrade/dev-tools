#!/usr/bin/env python3
"""
Claude Git Commit Assistant

This script uses Claude API to generate commit messages
based on git diffs. It extracts git diff information, passes it to
Claude API, and uses the generated message for a git commit.
It automatically stages all unstaged changes before generating the message.

Usage:
  claude_commit.py [options]

Basic Options:
  -h, --help            Show this help message and exit
  -m MODEL, --model MODEL
                        Specify the Claude model to use (default: claude-3-sonnet-20240229)
  ...
  -g, --gitmoji         Add gitmoji to the commit message
...
Examples:
  ...
  claude_commit -b feature/new-feature --pr

  # Create a compact commit message with automatic push
  claude_commit -s compact --push
...
Requirements:
  ...
  - Python 3.6+
  - Anthropic API access (set ANTHROPIC_API_KEY environment variable)
...
def check_dependencies():
    ...
    if not api_key:
        print_colored("Error: ANTHROPIC_API_KEY environment variable not set", RED)
        print_colored("Please set it with: export ANTHROPIC_API_KEY=your_api_key", YELLOW)
        sys.exit(1)
...
def create_prompt(diff, style, files, use_gitmoji=False, isDryRun=False):
    """Create the prompt for Claude based on the diff and style"""
    # Get the appropriate prompt template based on the style
    prompt_template = PROMPT_MAP.get(style, PROMPT_MAP["conventional"])
    
    # If gitmoji is requested, modify the instruction
    if use_gitmoji and "You will act as a git commit message generator" in prompt_template:
        prompt_template = prompt_template.replace(
            "[optional emoji]", 
            "emoji"
        )
    elif use_gitmoji:
        # Add gitmoji instruction to other styles
        prompt_template = prompt_template.replace(
            "You are a helpful assistant that generates", 
            "You are a helpful assistant that generates messages with a relevant emoji prefix. "
        )
...
def generate_commit_message(prompt, model):
    """Generate a commit message using the Claude API"""
    print_colored(f"Generating commit message using Claude {model}...", BLUE)
    ...
    try:
        ...
        message = result.get("content", [{"text": "No response from Claude"}])[0]["text"].strip()
        
        return message
    except requests.exceptions.RequestException as e:
        print_colored(f"Error communicating with Claude API: {e}", RED)
        if hasattr(e, 'response') and e.response:
            print_colored(f"Response: {e.response.text}", RED)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description='Generate git commit messages using Claude API')
    parser.add_argument('-m', '--model', default=DEFAULT_MODEL,
                      help=f'Specify the Claude model to use (default: {DEFAULT_MODEL})')
    ...
    parser.add_argument('-s', '--style', default=DEFAULT_STYLE, 
                      ...
                      help=f'Commit message style (default: {DEFAULT_STYLE})')
    ...
    parser.add_argument('-g', '--gitmoji', action='store_true',
    ...
    try:
        ...
        # Create prompt
        prompt = create_prompt(diff, args.style, files, args.gitmoji, args.dry_run)
